...
6. **Declare the rollback impact** — set or escalate the `**Migration:**` marker on `## [Unreleased]` in [`CHANGELOG.md`](../CHANGELOG.md) to `backward-compatible` if the previous app version still runs against the new schema, or `breaking` if it does not. Escalate only; never downgrade a marker an earlier PR already raised. You are the last person who will know the answer cheaply — there are no down migrations, so a `breaking` release can only be rolled back by restoring a pre-upgrade dump. The changelog gate enforces the marker at release time ([Issue #277](https://github.com/aellington89/finance-stack/issues/277)); the values are defined in [docs/releases.md](releases.md).
7. **Migrations apply automatically** on the next `docker compose up` via the `migrate` service. To apply manually against a running stack: `docker compose run --rm migrate`.
